...
u8 MPU6050_Set_Gyro_Fsr(u8 fsr);	//设置MPU6050陀螺仪传感器满量程范围
u8 MPU6050_Set_Accel_Fsr(u8 fsr);	//设置MPU6050加速度传感器满量程范围
u8 MPU6050_Set_LPF(u16 lpf);			//设置MPU6050的数字低通滤波器
u8 MPU6050_Set_Rate(u16 rate);		//设置MPU6050的采样率	
u8 MPU6050_Set_Fifo(u8 sens);


short MPU6050_Get_Temperature(void);//得到温度值
u8 MPU6050_Get_Gyroscope(short *gx,short *gy,short *gz);//得到陀螺仪值(原始值)
u8 MPU6050_Get_Accelerometer(short *ax,short *ay,short *az);//得到加速度值(原始值)
