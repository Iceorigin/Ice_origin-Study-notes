...
//#define delay_ms    delay_ms //(作用不明,可删除)
//#define get_ms      mget_ms  //(作用不明,可删除)
    ...
    data_sturct **data_more)
{
    ...
    if (i2c_read(st.hw->addr, st.reg->fifo_r_w, length, data))
        return -1;
    (*data_more)->more = fifo_count / length - 1;
    return 0;
}
...
u8 mpu_dmp_get_data(float *pitch,float *roll,float *yaw)
{
	...
	temp=mymalloc(1,sizeof(data_sturct));
	if(temp==NULL)
	{
		printf("DMP数据申请内存失败/r/n");
		return 1;
	}
	temp->q0=1.0f;
	temp->q1=0.0f;
	...
	if(dmp_read_fifo(&temp))
	{
		myfree(1,temp);
		return 2;	 
	}
	if(temp->sensors&INV_WXYZ_QUAT) 
	{
		temp->q0 = temp->quat[0] / q30;	//q30格式转换为浮点数
		temp->q1 = temp->quat[1] / q30;
		temp->q2 = temp->quat[2] / q30;
		temp->q3 = temp->quat[3] / q30; 
		//计算得到俯仰角/横滚角/航向角
		*pitch = asin(-2 * temp->q1 * temp->q3 + 2 * temp->q0* temp->q2)* 57.3;	// pitch
		*roll  = atan2(2 * temp->q2 * temp->q3 + 2 * temp->q0 * temp->q1, -2 * temp->q1 * temp->q1 - 2 * temp->q2* temp->q2 + 1)* 57.3;	// roll
		*yaw   = atan2(2*(temp->q1*temp->q2 + temp->q0*temp->q3),temp->q0*temp->q0+temp->q1*temp->q1-temp->q2*temp->q2-temp->q3*temp->q3) * 57.3;	//yaw
	}
	else
	{
		myfree(1,temp);
