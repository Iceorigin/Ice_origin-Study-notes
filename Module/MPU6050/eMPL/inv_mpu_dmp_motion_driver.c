...
int dmp_read_fifo(data_sturct** data)
{
    ...
    /* Get a packet. */
    if (mpu_read_fifo_stream(dmp.packet_length, fifo_data,data))
        return -1;
    ...
    if (dmp.feature_mask & (DMP_FEATURE_LP_QUAT | DMP_FEATURE_6X_LP_QUAT)) {
        ...
        long quat_q14[4], quat_mag_sq;
#endif
        (*data)->quat[0] = ((long)fifo_data[0] << 24) | ((long)fifo_data[1] << 16) |
            ((long)fifo_data[2] << 8) | fifo_data[3];
       (*data)->quat[1] = ((long)fifo_data[4] << 24) | ((long)fifo_data[5] << 16) |
            ((long)fifo_data[6] << 8) | fifo_data[7];
        (*data)->quat[2] = ((long)fifo_data[8] << 24) | ((long)fifo_data[9] << 16) |
            ((long)fifo_data[10] << 8) | fifo_data[11];
        (*data)->quat[3] = ((long)fifo_data[12] << 24) | ((long)fifo_data[13] << 16) |
            ((long)fifo_data[14] << 8) | fifo_data[15];
        ii += 16;
#ifdef FIFO_CORRUPTION_CHECK
        /* We can detect a corrupted FIFO by monitoring the quaternion data and
         ...
         * math.
         */
        quat_q14[0] = (*data)->quat[0] >> 16;
        quat_q14[1] = (*data)->quat[1] >> 16;
        quat_q14[2] = (*data)->quat[2] >> 16;
        quat_q14[3] = (*data)->quat[3] >> 16;
        quat_mag_sq = quat_q14[0] * quat_q14[0] + quat_q14[1] * quat_q14[1] +
            quat_q14[2] * quat_q14[2] + quat_q14[3] * quat_q14[3];
        if ((quat_mag_sq < QUAT_MAG_SQ_MIN) ||
            (quat_mag_sq > QUAT_MAG_SQ_MAX)) {
        ...
        (*data)->sensors |= INV_WXYZ_QUAT;
#endif
    }

    if (dmp.feature_mask & DMP_FEATURE_SEND_RAW_ACCEL) {
        (*data)->accel[0] = ((short)fifo_data[ii+0] << 8) | fifo_data[ii+1];
        (*data)->accel[1] = ((short)fifo_data[ii+2] << 8) | fifo_data[ii+3];
        (*data)->accel[2] = ((short)fifo_data[ii+4] << 8) | fifo_data[ii+5];
        ii += 6;
        (*data)->sensors|= INV_XYZ_ACCEL;
    }

    if (dmp.feature_mask & DMP_FEATURE_SEND_ANY_GYRO) {
        (*data)->gyro[0] = ((short)fifo_data[ii+0] << 8) | fifo_data[ii+1];
        (*data)->gyro[1] = ((short)fifo_data[ii+2] << 8) | fifo_data[ii+3];
        (*data)->gyro[2] = ((short)fifo_data[ii+4] << 8) | fifo_data[ii+5];
        ii += 6;
        (*data)->sensors |= INV_XYZ_GYRO;
    }
