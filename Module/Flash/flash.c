...
/*	4Kbytes为一个扇区(Sector)
 *	16个扇区(Sector)为1个块区(Block)
 *	64KB=1个Block
 *	每256字节为一个页
 *	EN25Q128
 *	容量为16M字节,共有256个块区(Block),4096个扇区(Sector) 
 *	初始化SPI FLASH的IO口
 */
...
{  
	u8 byte=0;   
	EN25QXX_CS=0; //使能器件   
	SPIx_ReadWriteByte(EN25X_ReadStatusReg);    //发送读取状态寄存器命令    
...
{   
	EN25QXX_CS=0; //使能器件   
	SPIx_ReadWriteByte(EN25X_WriteStatusReg);//发送写取状态寄存器命令    
...
{
	EN25QXX_CS=0; //使能器件   
  SPIx_ReadWriteByte(EN25X_WriteEnable); 	 //发送写使能  
	EN25QXX_CS=1; //取消片选     	      
}
...
{  
	EN25QXX_CS=0; //使能器件   
  SPIx_ReadWriteByte(EN25X_WriteDisable);  //发送写禁止指令    
	EN25QXX_CS=1; //取消片选     	      
} 
...
{ 
 	u16 i;   										    
	EN25QXX_CS=0; //使能器件   
  SPIx_ReadWriteByte(EN25X_ReadData); //发送读取命令   
...
void EN25QXX_Write_Page(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite)
{
  ...
  EN25QXX_Write_Enable(); //SET WEL 
	EN25QXX_CS=0; //使能器件   
  SPIx_ReadWriteByte(EN25X_PageProgram); //发送写页命令   
...
{                                   
  ...
  EN25QXX_Wait_Busy();   
 	EN25QXX_CS=0; //使能器件   
  SPIx_ReadWriteByte(EN25X_ChipErase); //发送片擦除命令  
	EN25QXX_CS=1; //取消片选     	      
...
{  
  ...
  EN25QXX_Wait_Busy();   
 	EN25QXX_CS=0;//使能器件   
  SPIx_ReadWriteByte(EN25X_SectorErase); //发送扇区擦除指令 
...
{ 
  EN25QXX_CS=0; //使能器件   
  SPIx_ReadWriteByte(EN25X_PowerDown); //发送掉电命令  
	EN25QXX_CS=1; //取消片选     	      
  delay_us(3); //等待TPD  
}   
...
{  
  EN25QXX_CS=0; //使能器件   
  SPIx_ReadWriteByte(EN25X_ReleasePowerDown); //  send EN25X_PowerDown command 0xAB    
	EN25QXX_CS=1; //取消片选     	      
  delay_us(3); //等待TRES1
}   
