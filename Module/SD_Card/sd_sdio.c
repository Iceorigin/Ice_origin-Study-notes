...
static u8 DeviceMode=0; //工作模式标志,注意,工作模式必须通过SD_SetDeviceMode,后才算数.这里只是定义一个默认的模式
...
SD_Error SD_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = SD_Pin_D0|SD_Pin_D1|SD_Pin_D2|SD_Pin_D3|SD_Pin_SCK;	//PC.8~12 复用输出
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; 		 //复用推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
	GPIO_Init(GPIOC, &GPIO_InitStructure);					 //根据设定参数初始化PC.8~12 
 
	GPIO_InitStructure.GPIO_Pin = SD_Pin_CMD;	//PD2 复用输出
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; 		 //复用推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
...
//	GPIO_Init(GPIOD, &GPIO_InitStructure);					 //根据设定参数初始化PD7
	...
	{
		#if debug
			printf("Error: #%d 设置SDIO总线宽度失败! \r\n",errorstatus);
		#endif
		return errorstatus;	
	}
	//SDIO传输频率
	if((errorstatus==SD_OK)||(CardType==SDIO_MULTIMEDIA_CARD))
	{  		    
		...
		errorstatus=SD_SetDeviceMode(SD_MODE);	//设置模式
		if(errorstatus!=SD_OK)
		{
			#if debug
				printf("Error: #%d 设置为模式失败! \r\n",errorstatus);
			#endif
			return errorstatus;	
		}
 	}
...
SD_Error SD_PowerON(void)
{
	...
	u32 SDType=SD_STD_CAPACITY;//默认小容量
	...
	for(i=0;i<74;i++)
	{
		SDIO_CmdInitStructure.SDIO_Argument = 0x0; //发送CMD0进入空闲状态模式命令.
		SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD0; //复位卡到空闲状态
	...
	SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;//接收短响应r7
  ...
  errorstatus=CmdResp7Error();						//等待R7响应
	if(errorstatus==SD_OK) 								//R7响应正常
	{
		...
		SDType=SD_HIGH_CAPACITY;			   			//区分大/小容量卡
	}
	
	SDIO_CmdInitStructure.SDIO_Argument = 0x00;//发送CMD55,短响应
	SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD55;	  //CMD55
	SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
	SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
	SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
	SDIO_SendCommand(&SDIO_CmdInitStructure);			//发送CMD55,短响应
	errorstatus=CmdResp1Error(SD_CMD55); 	 	//等待R1响应   
	if(errorstatus!=SD_OK)
	...
	{																  
		...
		//循环地发送ACMD41指令，循环一定次数。ACMD41命令的目的是给予SD卡控制器一个识别SD卡是否可以在所给Vdd范围下工作的机制。
		...
		{	   										   
			...
			//ACMD41，命令参数由支持的电压范围及HCS位组成，HCS位置一来区分卡是SDSC（低容量）还是SDHC(高容量)
			SDIO_CmdInitStructure.SDIO_Argument = SD_VOLTAGE_WINDOW_SD | SDType;	//发送ACMD41,短响应	
			SDIO_CmdInitStructure.SDIO_CmdIndex = SD_ACMD41;
			...
			validvoltage=(((response>>31)==1)?1:0); //判断SD卡上电是否完成（判断是否繁忙 0繁忙 1空闲 总是先收到卡状态的最高位）
			count++;
		}
		
		if(count>=SD_MAX_VOLT_TRIAL)
		{
			errorstatus=SD_INVALID_VOLTRANGE;//SD 无效电压
			return errorstatus;
		}	
		//区分SD卡容量
		if(response&=SD_HIGH_CAPACITY)
		{
			CardType=SDIO_HIGH_CAPACITY_SD_CARD;//大容量 SD 卡
		}
 	}
	else//MMC卡
	{
		...
		{	   										   				   
			...
			SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD1;
			SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short; //r3
			SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
			SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
			SDIO_SendCommand(&SDIO_CmdInitStructure);
		...
		CardType=SDIO_MULTIMEDIA_CARD;//MMC卡	  
  }  
  return(errorstatus);		
}
...
SD_Error SD_InitializeCards(void)
{
 	...
 	if(SDIO_SECURE_DIGITAL_IO_CARD!=CardType)			//非SECURE_DIGITAL_IO_CARD
	{
		...
		SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD2;
	...
	//SD卡2.0/1.1
	if((CardType==SDIO_STD_CAPACITY_SD_CARD_V1_1)||(CardType==SDIO_STD_CAPACITY_SD_CARD_V2_0)||(CardType==SDIO_SECURE_DIGITAL_IO_COMBO_CARD)||(CardType==SDIO_HIGH_CAPACITY_SD_CARD))//判断卡类型
	{
		//发送CMD3
		SDIO_CmdInitStructure.SDIO_Argument = 0x00;//发送CMD3,短响应 
		SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD3;	//cmd3
		SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short; //r6
		SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
		SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
		SDIO_SendCommand(&SDIO_CmdInitStructure);	//发送CMD3,短响应 
		
		errorstatus=CmdResp6Error(SD_CMD3,&rca); //等待R6响应 
		if(errorstatus!=SD_OK)
			return errorstatus; //响应错误		    
	}
	...
	if (CardType!=SDIO_SECURE_DIGITAL_IO_CARD)
	{
		...
		SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD9;
...
SD_Error SD_GetCardInfo(SD_CardInfo *cardinfo)
{
	...
	/*============== C S D =================*/
	//CSD结构 2数据位 6保留
	tmp=(u8)((CSD_Tab[0]&0xFF000000)>>24);							//CSD_Tab[0]第1个字节
	cardinfo->SD_csd.CSDStruct=(tmp&0xC0)>>6;						//CSD结构                                            [127:126]
	cardinfo->SD_csd.SysSpecVersion=(tmp&0x3C)>>2;			//2.0协议还没定义这部分(为保留),应该是后续协议定义的 [125:122]
	cardinfo->SD_csd.Reserved1=tmp&0x03;			  				//2个保留位                                          [121:120]
	
	tmp=(u8)((CSD_Tab[0]&0x00FF0000)>>16);							//CSD_Tab[0]第2个字节 
	cardinfo->SD_csd.TAAC=tmp;				   		  					//数据读访问时间      [119:112]
	
	tmp=(u8)((CSD_Tab[0]&0x0000FF00)>>8);	  						//CSD_Tab[0]第3个字节 [111:104]
	cardinfo->SD_csd.NSAC=tmp;		  			    					//时钟周期(NSAC*100)中的数据读访问时间
	
	tmp=(u8)(CSD_Tab[0]&0x000000FF);				   			    //CSD_Tab[0]第4个字节 
	cardinfo->SD_csd.MaxBusClkFrec=tmp;		     				  //最大数据传输速率	  [103:96]		
	
	tmp=(u8)((CSD_Tab[1]&0xFF000000)>>24);							//CSD_Tab[1]第1个字节 [95:84]
	cardinfo->SD_csd.CardComdClasses=tmp<<4;   					//卡指令类高四位      [92:89]
	
	tmp=(u8)((CSD_Tab[1]&0x00FF0000)>>16);	 						//CSD_Tab[1]第2个字节 
	cardinfo->SD_csd.CardComdClasses|=(tmp&0xF0)>>4;		//卡指令类低四位      [88:84]
	cardinfo->SD_csd.RdBlockLen=tmp&0x0F;	    					//最大读取数据长度    [83:80]
	
	tmp=(u8)((CSD_Tab[1]&0x0000FF00)>>8);					    	//CSD_Tab[1]第3个字节
	cardinfo->SD_csd.PartBlockRead=(tmp&0x80)>>7;		    //允许分块读      		[79]
	cardinfo->SD_csd.WrBlockMisalign=(tmp&0x40)>>6;	    //写块不对齐     		  [78] 
	cardinfo->SD_csd.RdBlockMisalign=(tmp&0x20)>>5;   	//读块不对齐    		  [77]
	cardinfo->SD_csd.DSRImpl=(tmp&0x10)>>4;             //执行的 DSR     			[76]
	cardinfo->SD_csd.Reserved2=0; 									    //保留       					[75:71]           
	
	//标准1.1/2.0卡/MMC卡
 	if((CardType==SDIO_STD_CAPACITY_SD_CARD_V1_1)||(CardType==SDIO_STD_CAPACITY_SD_CARD_V2_0)||(CardType==SDIO_MULTIMEDIA_CARD))
	{
		cardinfo->SD_csd.DeviceSize=(tmp&0x03)<<10;				//C_SIZE(12位)
	 	tmp=(u8)(CSD_Tab[1]&0x000000FF); 									//CSD_Tab[1]第4个字节	
		cardinfo->SD_csd.DeviceSize|=(tmp)<<2;						
		
 		tmp=(u8)((CSD_Tab[2]&0xFF000000)>>24);						//CSD_Tab[2]第1个字节	
		cardinfo->SD_csd.DeviceSize|=(tmp&0xC0)>>6;
 		cardinfo->SD_csd.MaxRdCurrentVDDMin=(tmp&0x38)>>3;//最小读取电压
		cardinfo->SD_csd.MaxRdCurrentVDDMax=(tmp&0x07);		//最大读取电压
		
 		tmp=(u8)((CSD_Tab[2]&0x00FF0000)>>16);						//CSD_Tab[2]第2个字节	
		cardinfo->SD_csd.MaxWrCurrentVDDMin=(tmp&0xE0)>>5;//最小写入电压
		cardinfo->SD_csd.MaxWrCurrentVDDMax=(tmp&0x1C)>>2;//最大读取电压
		cardinfo->SD_csd.DeviceSizeMul=(tmp&0x03)<<1;			//C_SIZE_MULT设备尺寸乘数高2位
		
 		tmp=(u8)((CSD_Tab[2]&0x0000FF00)>>8);	  					//CSD_Tab[2]第3个字节	
		cardinfo->SD_csd.DeviceSizeMul|=(tmp&0x80)>>7;		//C_SIZE_MULT设备尺寸乘数
	...
	{
 		tmp=(u8)(CSD_Tab[1]&0x000000FF); 									//CSD_Tab[1]第4个字节	
		cardinfo->SD_csd.DeviceSize=(tmp&0x3F)<<16;				//C_SIZE 设备尺寸 		[69:62]
		
 		tmp=(u8)((CSD_Tab[2]&0xFF000000)>>24); 						//CSD_Tab[2]第1个字节	
 		cardinfo->SD_csd.DeviceSize|=(tmp<<8);						//C_SIZE 设备尺寸 		[61:54]	
		
 		tmp=(u8)((CSD_Tab[2]&0x00FF0000)>>16);						//CSD_Tab[2]第2个字节	
 		cardinfo->SD_csd.DeviceSize|=(tmp);								//C_SIZE 设备尺寸 		[53:48]	
  ...
  tmp=(u8)((CSD_Tab[2]&0x0000FF00)>>8); 							//CSD_Tab[2]第3个字节
	cardinfo->SD_csd.EraseGrSize=(tmp&0x40)>>6;					//擦除组大小      		[46]
	cardinfo->SD_csd.EraseGrMul=(tmp&0x3F)<<1;	        //擦除组大小乘数  		[45:38]
  
	tmp=(u8)(CSD_Tab[2]&0x000000FF);										//CSD_Tab[2]第4个字节
	cardinfo->SD_csd.EraseGrMul|=(tmp&0x80)>>7;					//擦除组大小乘数  		[39]
	cardinfo->SD_csd.WrProtectGrSize=(tmp&0x7F);				//写保护组大小    		[38:32]
	
 	tmp=(u8)((CSD_Tab[3]&0xFF000000)>>24);							//CSD_Tab[3]第1个字节	
	cardinfo->SD_csd.WrProtectGrEnable=(tmp&0x80)>>7;		//写保护组使能    		[31]
	cardinfo->SD_csd.ManDeflECC=(tmp&0x60)>>5;					//制造商默认 ECC 			[30:29]
	cardinfo->SD_csd.WrSpeedFact=(tmp&0x1C)>>2;					//写速度系数     			[28:26]
	cardinfo->SD_csd.MaxWrBlockLen=(tmp&0x03)<<2;	 			//最大写数据块长度		[25:24]
	
	tmp=(u8)((CSD_Tab[3]&0x00FF0000)>>16);							//CSD_Tab[3]第2个字节
	cardinfo->SD_csd.MaxWrBlockLen|=(tmp&0xC0)>>6;			//最大写数据块长度		[23:22]
	cardinfo->SD_csd.WriteBlockPaPartial=(tmp&0x20)>>5; //允许块部分写        [21]
	cardinfo->SD_csd.Reserved3=0;												//保留                [20:17]
	cardinfo->SD_csd.ContentProtectAppli=(tmp&0x01);  	//内容保护应用        [16]
	
	tmp=(u8)((CSD_Tab[3]&0x0000FF00)>>8);								//CSD_Tab[3]第3个字节
	cardinfo->SD_csd.FileFormatGrouop=(tmp&0x80)>>7;		//文件格式组          [15]
	cardinfo->SD_csd.CopyFlag=(tmp&0x40)>>6;						//拷贝标志(OTP)				[14]
	cardinfo->SD_csd.PermWrProtect=(tmp&0x20)>>5;				//永久写保护          [13]
	cardinfo->SD_csd.TempWrProtect=(tmp&0x10)>>4;				//临时写保护          [12]
	cardinfo->SD_csd.FileFormat=(tmp&0x0C)>>2;					//文件格式            [11:10]
	cardinfo->SD_csd.ECC=(tmp&0x03); 										//ECC码/保留					[9:8]

	tmp=(u8)(CSD_Tab[3]&0x000000FF);										//CSD_Tab[3]第4个字节
	cardinfo->SD_csd.CSD_CRC=(tmp&0xFE)>>1;							//CSD CRC校验码				[7:1]
	cardinfo->SD_csd.Reserved4=1;		 										//无用，总是 1				[0]
	
	/*============== C I D =================*/
	tmp=(u8)((CID_Tab[0]&0xFF000000)>>24);							//CID_Tab[0]第1个字节
	cardinfo->SD_cid.ManufacturerID=tmp;								//制造商 ID 					[127:120]
	
	tmp=(u8)((CID_Tab[0]&0x00FF0000)>>16);							//CID_Tab[0]第2个字节
	cardinfo->SD_cid.OEM_AppliID=tmp<<8;	  						//OEM/应用 ID         [119:112]
	tmp=(u8)((CID_Tab[0]&0x000000FF00)>>8);							//CID_Tab[0]第3个字节
	cardinfo->SD_cid.OEM_AppliID|=tmp;	    						//OEM/应用 ID         [111:104]
	
	//产品名称第 1 部分 [103:72]
	tmp=(u8)(CID_Tab[0]&0x000000FF);										//CID_Tab[0]第4个字节	
	cardinfo->SD_cid.ProdName1=tmp<<24;									
	tmp=(u8)((CID_Tab[1]&0xFF000000)>>24); 							//CID_Tab[1]第1个字节
	cardinfo->SD_cid.ProdName1|=tmp<<16;								
	tmp=(u8)((CID_Tab[1]&0x00FF0000)>>16);	   					//CID_Tab[1]第2个字节
	cardinfo->SD_cid.ProdName1|=tmp<<8;										 
	tmp=(u8)((CID_Tab[1]&0x0000FF00)>>8);								//CID_Tab[1]第3个字节
	cardinfo->SD_cid.ProdName1|=tmp;									   
	tmp=(u8)(CID_Tab[1]&0x000000FF);	  								//CID_Tab[1]第4个字节
	cardinfo->SD_cid.ProdName2=tmp;											//产品名称第 2部分   	[71:64]		

	
	tmp=(u8)((CID_Tab[2]&0xFF000000)>>24); 							//CID_Tab[2]第1个字节
	cardinfo->SD_cid.ProdRev=tmp;		 										//产品版本           	[65:56] 
	
	//产品序列号 [55:24]
	tmp=(u8)((CID_Tab[2]&0x00FF0000)>>16);							//CID_Tab[2]第2个字节
	cardinfo->SD_cid.ProdSN=tmp<<24;	   								
	tmp=(u8)((CID_Tab[2]&0x0000FF00)>>8); 							//CID_Tab[2]第3个字节
	cardinfo->SD_cid.ProdSN|=tmp<<16;	   
	tmp=(u8)(CID_Tab[2]&0x000000FF);   									//CID_Tab[2]第4个字节
	cardinfo->SD_cid.ProdSN|=tmp<<8;		   
	tmp=(u8)((CID_Tab[3]&0xFF000000)>>24); 							//CID_Tab[3]第1个字节
	cardinfo->SD_cid.ProdSN|=tmp;		
	
	tmp=(u8)((CID_Tab[3]&0x00FF0000)>>16);	 						//CID_Tab[3]第2个字节
	cardinfo->SD_cid.Reserved1|=(tmp&0xF0)>>4;					//保留                [23:20]
	cardinfo->SD_cid.ManufactDate=(tmp&0x0F)<<8; 				//生产日期  					[19:16] 
	tmp=(u8)((CID_Tab[3]&0x0000FF00)>>8);								//CID_Tab[3]第3个字节
	cardinfo->SD_cid.ManufactDate|=tmp;									//生产日期  					[15:8]
		
	tmp=(u8)(CID_Tab[3]&0x000000FF);										//CID_Tab[3]第4个字节
	cardinfo->SD_cid.CID_CRC=(tmp&0xFE)>>1;							//CRC7 校验码					[7:1]
	cardinfo->SD_cid.Reserved2=1;	 											//不使用，总是 1			[0]
...
SD_Error SD_SelectDeselect(u32 addr)
{
	...
	SDIO_CmdInitStructure.SDIO_CmdIndex=SD_CMD7;
	SDIO_CmdInitStructure.SDIO_Response=SDIO_Response_Short;
	SDIO_CmdInitStructure.SDIO_Wait=SDIO_Wait_No;
	SDIO_CmdInitStructure.SDIO_CPSM=SDIO_CPSM_Enable;
	SDIO_SendCommand(&SDIO_CmdInitStructure);//发送CMD7,选择卡,短响应
	
 	return CmdResp1Error(SD_CMD7);	 
...
SD_Error SD_EnableWideBusOperation(u32 wmode)
{
  SD_Error errorstatus=SD_OK;
 	...
 	else if((CardType==SDIO_STD_CAPACITY_SD_CARD_V1_1)||(CardType==SDIO_STD_CAPACITY_SD_CARD_V2_0)||(CardType==SDIO_HIGH_CAPACITY_SD_CARD))
	{
		...
		{
			errorstatus=SDEnWideBus(wmode);//使能宽总线模式
 			if(SD_OK==errorstatus)
			{
//				SDIO->CLKCR&=~(3<<11);		//清除之前的位宽设置    
//				SDIO->CLKCR|=(u16)wmode<<11;//1位/4位总线宽度 
//				SDIO->CLKCR|=0<<14;			//不开启硬件流控制
				SDIO_InitStructure.SDIO_BusWide = (wmode>=2? SDIO_BusWide_1b:SDIO_BusWide_4b); //1位/4位总线宽度
				SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable;//不开启硬件流控制
				SDIO_Init(&SDIO_InitStructure);
			}
		}  
	}
...
SD_Error SDEnWideBus(u8 enx)
{
	...
	if((scr[1]&SD_WIDE_BUS_SUPPORT)!=SD_ALLZERO)		//支持宽总线
	{
		...
		SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD55;
		SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;//r1
		SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
		SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
		SDIO_SendCommand(&SDIO_CmdInitStructure);

		errorstatus=CmdResp1Error(SD_CMD55);
		if(errorstatus!=SD_OK)
			return errorstatus; 

		SDIO_CmdInitStructure.SDIO_Argument = arg;//发送ACMD6,短响应,参数:10,4位;00,1位.	
		SDIO_CmdInitStructure.SDIO_CmdIndex = SD_ACMD6;
		SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;//r1
		SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
		SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
		SDIO_SendCommand(&SDIO_CmdInitStructure);

		errorstatus=CmdResp1Error(SD_ACMD6);
		return errorstatus;
	}
	else 
...
{ 
	...
	//发送CMD16，设置所有块命令的长度
	SDIO_CmdInitStructure.SDIO_Argument = (u32)0x8;	 //发送CMD16,短响应,设置Block Size为8字节	
	SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD16; //	 cmd16
	SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short; //r1
	SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
	SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
	SDIO_SendCommand(&SDIO_CmdInitStructure);

	errorstatus=CmdResp1Error(SD_CMD16);
	if(errorstatus!=SD_OK)
		return errorstatus;	   
	
	//发送CMD55
	SDIO_CmdInitStructure.SDIO_Argument = (u32)RCA<<16; 
	SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD55;//发送CMD55,短响应 	
	SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short; //r1
	SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
	SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
	SDIO_SendCommand(&SDIO_CmdInitStructure);
	...
	SDIO_DataInitStructure.SDIO_DataTimeOut = SD_DATATIMEOUT;//数据状态机超时时间设置
	...
	SDIO_DataInitStructure.SDIO_TransferDir = SDIO_TransferDir_ToSDIO;//数据传输方向：SD卡至控制器
	SDIO_DataInitStructure.SDIO_TransferMode = SDIO_TransferMode_Block;//数据传输模式：块数据传输
	SDIO_DataInitStructure.SDIO_DPSM = SDIO_DPSM_Enable;//数据传输使能位
	SDIO_DataConfig(&SDIO_DataInitStructure);		
	
	//发送ACMD51
	SDIO_CmdInitStructure.SDIO_Argument = 0x0;
	SDIO_CmdInitStructure.SDIO_CmdIndex = SD_ACMD51;	//发送ACMD51,短响应,参数为0	
	SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;  //r1
	SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
	SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
	SDIO_SendCommand(&SDIO_CmdInitStructure);
	
 	errorstatus=CmdResp1Error(SD_ACMD51);
	if(errorstatus!=SD_OK)
		return errorstatus;			
	
	//接收FIFO上溢错误，已发送/接收数据块(CRC检测失败)，数据超时，已发送/接收数据块(CRC检测成功)，在宽总线模式，没有在所有数据信号上检测到起始位
	...
	//先进先出原则 一次传输8字节  1个32位数据块
	//[15]擦除后数据状态 	 [14:12]SD 安全支持 [11:8]SD 卡支持总线宽度
	//[7:4]SCR 结构体      [3:1]SD 卡规范版本
	//把数据顺序按8位为单位倒过来.   	
...
SD_Error SD_SendStatus(uint32_t *pcardstatus)
{
	...
	SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD13;
	SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;//r1
	SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
	SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
	SDIO_SendCommand(&SDIO_CmdInitStructure);	

	errorstatus=CmdResp1Error(SD_CMD13);	//查询响应状态 
	if(errorstatus!=SD_OK)
		return errorstatus;
...
{	  
  ...
  //初始化 SDIO 数据路径
  SDIO_DataInitStructure.SDIO_DataBlockSize= SDIO_DataBlockSize_1b ; //设置数据块长度
	SDIO_DataInitStructure.SDIO_DataLength= 0 ;												 //需要传输的数据字节长度。
	SDIO_DataInitStructure.SDIO_DataTimeOut=SD_DATATIMEOUT ; 					 //数据状态机超时时间设置
	SDIO_DataInitStructure.SDIO_DPSM=SDIO_DPSM_Enable;								 //数据传输使能
	SDIO_DataInitStructure.SDIO_TransferDir=SDIO_TransferDir_ToCard;   //控制器至卡
	SDIO_DataInitStructure.SDIO_TransferMode=SDIO_TransferMode_Block;  //数据传输模式 块数据传输
  SDIO_DataConfig(&SDIO_DataInitStructure);
	...
	{
		power=convert_from_bytes_to_power_of_two(blksize); //得到NumberOfBytes以2为底的指数.    
		
		//发送CMD16 设置的数据块大小
		SDIO_CmdInitStructure.SDIO_Argument = blksize;
		SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD16;
		SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
		SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
		SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
		SDIO_SendCommand(&SDIO_CmdInitStructure);//发送CMD16+设置数据长度为blksize,短响应
		
		errorstatus=CmdResp1Error(SD_CMD16);	//等待R1响应  
		if(errorstatus!=SD_OK)
			return errorstatus; //响应错误	
	}
	else 
	...
	SDIO_DataInitStructure.SDIO_TransferDir=SDIO_TransferDir_ToSDIO; //卡至控制器
	SDIO_DataInitStructure.SDIO_TransferMode=SDIO_TransferMode_Block;
  SDIO_DataConfig(&SDIO_DataInitStructure);
	
	
	//发送CMD17 从addr地址出读取数据
	SDIO_CmdInitStructure.SDIO_Argument =  addr;
  SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD17;
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short; //r1
  SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
  SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
  SDIO_SendCommand(&SDIO_CmdInitStructure);//发送CMD17+,短响应 
	
	errorstatus=CmdResp1Error(SD_CMD17);//等待R1响应   
	if(errorstatus!=SD_OK)
		return errorstatus;   		//响应错误	 
	...
	else if(DeviceMode==SD_DMA_MODE)
	{
  	SD_DMA_Config((u32*)buf,blksize,DMA_DIR_PeripheralSRC); 
		...
		//DMA2 通道4是否了传输完成事件/传输结束标置位/数据传输错误标志/数据超时
...
SD_Error SD_ReadMultiBlocks(u8 *buf,long long addr,u16 blksize,u32 nblks)
{
  ...
  SDIO->DCTRL=0x0;		//数据控制寄存器清零(关DMA)   
	...
	//清除DPSM状态机配置
	SDIO_DataInitStructure.SDIO_DataBlockSize= 0 ; //设置数据块长度
	SDIO_DataInitStructure.SDIO_DataLength= 0 ;												 //需要传输的数据字节长度。
	SDIO_DataInitStructure.SDIO_DataTimeOut=SD_DATATIMEOUT ; 					 //数据状态机超时时间设置
	SDIO_DataInitStructure.SDIO_DPSM=SDIO_DPSM_Enable;								 //数据传输使能
	SDIO_DataInitStructure.SDIO_TransferDir=SDIO_TransferDir_ToCard;   //控制器至卡
  ...
  SDIO_DataConfig(&SDIO_DataInitStructure);
    ...
    SDIO_DataInitStructure.SDIO_DataBlockSize= power<<4; ;//nblks*blksize,512块大小,卡到控制器 详情查看寄存器SDIO_DCTRL
    ...
    SDIO_DataInitStructure.SDIO_DataTimeOut=SD_DATATIMEOUT ;
	  ...
	  //发送CMD18+从addr地址出读取数据
	  SDIO_CmdInitStructure.SDIO_Argument =  addr;//发送CMD18+从addr地址出读取数据,短响应 
	  SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD18;
	  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
	  SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
	  SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
	  SDIO_SendCommand(&SDIO_CmdInitStructure);	
		
	  errorstatus=CmdResp1Error(SD_CMD18);//等待R1响应   
	  if(errorstatus!=SD_OK)
		 return errorstatus;   	//响应错误	
		
		if(DeviceMode==SD_POLLING_MODE)//查询模式
		{
			INTX_DISABLE();//关闭总中断(POLLING模式,严禁中断打断SDIO读写操作!!!)
			...
			{
				if((CardType==SDIO_STD_CAPACITY_SD_CARD_V1_1)||(CardType==SDIO_STD_CAPACITY_SD_CARD_V2_0)||(CardType==SDIO_HIGH_CAPACITY_SD_CARD))
				{
					//发送CMD12  结束传输
					SDIO_CmdInitStructure.SDIO_Argument =  0;//发送CMD12+结束传输
					SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD12;
					SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
					SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
					SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
					SDIO_SendCommand(&SDIO_CmdInitStructure);	
					
					errorstatus=CmdResp1Error(SD_CMD12);//等待R1响应   
					if(errorstatus!=SD_OK)
						return errorstatus;	 
				}
			}
...
SD_Error SD_WriteBlock(u8 *buf,long long addr,  u16 blksize)
{
  ...
  SDIO_DataConfig(&SDIO_DataInitStructure);
	...
	errorstatus=CmdResp1Error(SD_CMD13);		//等待R1响应 
	if(errorstatus!=SD_OK)
		return errorstatus;
	...
	//检查READY_FOR_DATA位是否置位 见文档表4-34 
	while(((cardstatus&SD_OCR_READY_FOR_DATA)==0)&&(timeout>0)) 	//总线上表示缓存空的状态，即可以接受数据
	{
		timeout--;
		//发送CMD13,查询卡的状态
		SDIO_CmdInitStructure.SDIO_Argument = (u32)RCA<<16;//发送CMD13,查询卡的状态,短响应
	...
	SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD24;
	SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
	SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
	SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
	SDIO_SendCommand(&SDIO_CmdInitStructure);	
	
	errorstatus=CmdResp1Error(SD_CMD24);//等待R1响应   		   
	if(errorstatus!=SD_OK)
		return errorstatus; 
	...
	if (DeviceMode == SD_POLLING_MODE)//查询模式
	{
		...
		while(!(SDIO->STA&((1<<1)|(1<<3)|(1<<4)|(1<<9)|(1<<10))))//数据块发送成功/下溢/CRC/超时/起始位错误
		{
	...
	else if(DeviceMode==SD_DMA_MODE) //DMA模式
	{
 	...
 	errorstatus=IsCardProgramming(&cardstate);//检查卡是否正在执行写操作
  ...
  SDIO_DataConfig(&SDIO_DataInitStructure);
	...
	{					  
		...
		if((CardType==SDIO_STD_CAPACITY_SD_CARD_V1_1)||(CardType==SDIO_STD_CAPACITY_SD_CARD_V2_0)||(CardType==SDIO_HIGH_CAPACITY_SD_CARD))
		{
			...
			SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD23;
			SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
			SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
			SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
			SDIO_SendCommand(&SDIO_CmdInitStructure);
				
			errorstatus=CmdResp1Error(SD_CMD23);//等待R1响应 
			if(errorstatus!=SD_OK)
				return errorstatus;		
		}
		...
		SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD25;
		SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
		SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
		SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
		SDIO_SendCommand(&SDIO_CmdInitStructure);	
		
 		errorstatus=CmdResp1Error(SD_CMD25);	//等待R1响应   		   
		if(errorstatus!=SD_OK)
			return errorstatus;
...
SD_Error IsCardProgramming(u8 *pstatus)
{
	...
	SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD16;//发送CMD13 	
	SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
	SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
	SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
	SDIO_SendCommand(&SDIO_CmdInitStructure);	
 ...
 *	mbuf:存储器地址
 *	bufsize:传输数据量
 *	DMA_DIR:方向; DMA_DIR_PeripheralDST,存储器-->SDIO(写数据);
 ...
 *	返回值:sd卡错误码
 */
SD_Error Cmd0Error(void)
{
	SD_Error errorstatus = SD_OK;
	u32 timeout=SDIO_CMD0TIMEOUT;	//CMD0超时时间 
	...
	if(timeout==0)
		return SD_CMD_RSP_TIMEOUT; // 命令响应超时
...
SD_Error CmdResp1Error(u8 cmd)
{
	...
	while(1)
	{		
		status=SDIO->STA;//状态寄存器
		if(status&((1<<0)|(1<<2)|(1<<6)))break;//CRC错误/命令响应超时/已经收到响应(CRC校验成功)
	}
	
	if(SDIO_GetFlagStatus(SDIO_FLAG_CTIMEOUT) == SET)//响应超时					
	{																				    
 		SDIO_ClearFlag(SDIO_FLAG_CTIMEOUT);//清除命令响应超时标志
		return SD_CMD_RSP_TIMEOUT;
	}	
	
	if(SDIO_GetFlagStatus(SDIO_FLAG_CCRCFAIL) == SET) //CRC错误				
	{																				    
 		SDIO_ClearFlag(SDIO_FLAG_CCRCFAIL); 				//清除标志
		return SD_CMD_CRC_FAIL;
	}		
	if(cmd!=SDIO_GetCommandResponse())
...
SD_Error CmdResp6Error(u8 cmd,u16*prca)
{
	...
	if(cmd!=SDIO->RESPCMD) //判断是否响应cmd命令
	{
 		return SD_ILLEGAL_CMD; 		
	}	    
	SDIO_ClearFlag(SDIO_STATIC_FLAGS);//清除所有标记
	
	rspr1=(u32)SDIO->RESP1; //得到响应 	 
...
SD_Error SD_ProcessIRQSrc(void)
{
	...
	/*接收完成状态*/
	if(SDIO->STA&(1<<8))//接收完成状态 SDIO_GetFlagStatus(SDIO_FLAG_DATAEND)
	{	 
		if (StopCondition==1)
		{
			//发送CMD12  结束传输 	 
			...
			TransferError=CmdResp1Error(SD_CMD12);
		}
		else 
			TransferError = SD_OK;	
		
		//SDIO->ICR|=1<<8;清除完成状态标记
 		SDIO_ClearFlag(SDIO_FLAG_DATAEND);//清除完成状态标记
		
		//关闭相关中断
		//数据块CRC检测失败产生中断/数据超时产生中断/发送FIFO下溢错误产生中断/接收FIFO上溢错误产生中断
		//数据传输结束产生中断/起始位错误产生中断/发送FIFO半空产生中断/接收FIFO半满产生中断
		SDIO->MASK&=~((1<<1)|(1<<3)|(1<<4)|(1<<5)|(1<<8)|(1<<9)|(1<<14)|(1<<15));
		
 		TransferEnd = 1; //传输结束标志
		return (TransferError);
	}
	
	/*数据CRC错误/数据超时错误/FIFO上溢错误/FIFO下溢错误/起始位错误*/
