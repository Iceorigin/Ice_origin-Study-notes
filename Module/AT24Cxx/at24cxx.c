...
{				  
  ...
  IIC_Start();  
	if(EE_TYPE>AT24C16)
	{
		IIC_Send_Byte(EE_TYPE_ADDR);//发送写命令
		IIC_Wait_Ack();
		IIC_Send_Byte(ReadAddr>>8);//发送高地址	    
	}
	else 
	{
		IIC_Send_Byte(EE_TYPE_ADDR+((ReadAddr/256)<<1));//发送器件地址0XA0,写数据
	} 	
	IIC_Wait_Ack(); 
  IIC_Send_Byte(ReadAddr%256);//发送低地址
	IIC_Wait_Ack();	    
	IIC_Start();  	 	   
	IIC_Send_Byte(EE_TYPE_ADDR+1);//进入接收模式			   
	IIC_Wait_Ack();	 
...
{				   	  	    																 
  IIC_Start();  
	...
	{
		IIC_Send_Byte(EE_TYPE_ADDR+((WriteAddr/256)<<1));   //发送器件地址0XA0,写数据
	} 	 
	IIC_Wait_Ack();	   
