...
//内存管理参数
const u32 mem_table_size[SRAMBANK]={MEM1_ALLOC_TABLE_SIZE,MEM2_ALLOC_TABLE_SIZE};	//内存表大小
const u32 mem_block_size[SRAMBANK]={MEM1_BLOCK_SIZE,MEM2_BLOCK_SIZE};				    	//内存分块大小
const u32 mem_size[SRAMBANK]={MEM1_MAX_SIZE,MEM2_MAX_SIZE};						           	//内存总大小
...
{  
	my_mem_set(malloc_cortol.membase[memx], 0,mem_size[memx]);	//内存池所有数据清零
  my_mem_set(malloc_cortol.memmap[memx], 0,mem_table_size[memx]*2);//内存状态表数据清零    
...
{  
    ...
    if(!malloc_cortol.memrdy[memx])
			malloc_cortol.init(memx);//未初始化,先执行初始化 
	
    if(size==0)
			return 0XFFFFFFFF;//不需要分配
	
    xmemb=size/mem_block_size[memx];  	//获取需要分配的连续内存块数
